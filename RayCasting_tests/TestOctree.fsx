//
//  Like Test_PreprocessorGrid.fsx with octree
//
...
#load "Octree.fs"
#load "RayCoreOctree.fs"
#load "RayCoreOctree.fs"
#load "RayColorOctree.fs"
...
//Spheres
let ball = {center=Point3D(1.0,0.320,-0.50); radius=0.250; material=refractive } //0.7,0.2,0.0  Originalcenter=Point3D(1.0,0.20,-0.50)
...
//let path = @"C:\Users\JoseM\OneDrive\Phd\render\ray casting\RayCastingTest\Ray-Casting-Test\MeshSamples\pyramid.obj"
...
let all = {Meshes = [mesh2;mesh1];Sphere = [ball;ball2]; Cylinder=[]} //  -  mesh2;   
//let all = {Meshes = [];Sphere = [ball2]}
let lights ={Point= [];Circle = [clight]} //light;light2;light3;light4;light5
//partition.
let octree =  OctreeSubdivision (all, WorldLimitsExtend (all),50,0,5)
...
let casting (pixH:int list) =
    ...
    for i in 0..(PixNumW-1) do 
        for j in pixH do
                ...
                let intersects = Cast_Octree (Scene,Ray,octree)//CastRay_nest (Scene, Ray)
                match intersects with
                | [] -> rimage.[i,j] <- 0.0
                        gimage.[i,j] <- 0.0
                        bimage.[i,j] <- 0.0 //bmp.SetPixel(i,j,Color.Gray)
                | _ ->  let color = GlobalIllum_Octree(intersects |>List.minBy(fun x -> x.t), Scene ,octree )//colorAt (intersects |>List.minBy(fun x -> x.t), Scene  )
                        //let color = colorAtOctree(intersects |>List.minBy(fun x -> x.t), Scene ,octree )    //(fun x -> x) |> |> List.head S
...
//bmp.Save(@"C:\Users\JoseM\Desktop\test_AllScene_Octree.jpg")
